...
self.addEventListener('message', function(event) {
    // extract data
    var params = event.data.query;

    // Create the client object
    var client = new amps.Client(event.data.clientName);
    client.errorHandler(function(err) { self.postMessage({error: err}); });

    // let's go!
    client.connect(event.data.uri)
        .then(function() {
            var nextId = 1;
            var sowKeyMap;
            var header;
            ...
            return client.sowAndSubscribe(function(message) {
                ...
                options: 'oof,skip_n=0,' + (params.options ? (',' + params.options) : ''),
                topN: params.topN || 10,
